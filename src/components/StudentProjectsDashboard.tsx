...
export default function StudentProjectsDashboard({
  user, classroomName, projects, onOpen, onCreate, onDelete, onLogout,
}: Props) {
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <span className="block text-[9px] font-bold uppercase tracking-[0.18em] text-black/40">Meus projetos 5I's</span>
              <h1 className="text-base sm:text-lg font-bold truncate">Olá, {user.name}</h1>
              {classroomName && <p className="text-xs text-neutral-500 truncate">{classroomName}</p>}
            </div>
          </div>
          <button onClick={onLogout} className="shrink-0 p-2 sm:px-3 sm:py-2 rounded-xl border border-[#DDD] bg-white text-neutral-700 flex items-center gap-2 text-xs font-mono font-bold uppercase cursor-pointer">
            <ArrowLeft size={15} /><span className="hidden sm:inline">Sair</span>
          </button>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-8 py-6 sm:py-10 pb-[calc(2rem+env(safe-area-inset-bottom))]">
        <section className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold mt-1">Escolha onde continuar</h2>
            <p className="text-sm text-neutral-500 mt-1">Abra um projeto existente ou inicie uma nova investigação.</p>
          </div>
        ...
        {ordered.length === 0 ? (
          <section className="min-h-[360px] border-2 border-dashed border-[#DDD] rounded-3xl flex flex-col items-center justify-center text-center p-8 bg-white">
            ...
            <h3 className="text-xl font-bold">Você ainda não possui projetos</h3>
            <p className="text-sm text-neutral-500 max-w-md mt-2">Crie sua primeira mesa para começar a percorrer Ideação, Inambulação, Instauração, Inspeção e Implementação.</p>
