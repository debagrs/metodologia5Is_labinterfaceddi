...
import { ArrowLeft, GraduationCap, User, Users, Check, Mail, Lock, Loader2 } from 'lucide-react';
...
export default function LoginScreen({ classrooms, onLogin }: LoginScreenProps) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const session = await authenticate({ action: mode, name, email, password, role, classroomId: role === 'student' ? classroomId : undefined, partnerType: role === 'partner' ? partnerType : undefined, institution });
  ...
  return <div className="min-h-screen flex items-center justify-center bg-[#FDFDFB] p-4">
    <div className="w-full max-w-xl bg-white border border-[#E0E0DE] rounded-3xl shadow-xl p-6 sm:p-10">
      ...
      <div className="grid grid-cols-2 bg-[#F3F3F1] p-1 rounded-xl mb-6">
        <button onClick={()=>{setMode('login');setRole(null);setError('')}} className={`py-2 rounded-lg text-xs font-bold cursor-pointer ${mode==='login'?'bg-white shadow-sm':'text-neutral-500'}`}>ENTRAR</button>
        <button onClick={()=>{setMode('register');setError('')}} className={`py-2 rounded-lg text-xs font-bold cursor-pointer ${mode==='register'?'bg-white shadow-sm':'text-neutral-500'}`}>CRIAR CONTA</button>
      </div>
      ...
      {mode==='register' && !role ? <div>
        ...
        {mode==='register' && <button type="button" onClick={()=>setRole(null)} className="text-xs font-bold flex items-center gap-1 text-neutral-500 cursor-pointer"><ArrowLeft size={14}/> Trocar perfil</button>}
        ...
        {mode==='register' && role==='student' && <label className="block"><span className="text-[11px] font-mono font-bold uppercase text-neutral-500">Turma</span><select value={classroomId} onChange={e=>setClassroomId(e.target.value)} className="mt-1 w-full border border-[#E0E0DE] rounded-xl p-3 text-sm">{classrooms.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>}
        ...
        <button disabled={loading} className="w-full bg-black text-white rounded-full py-3.5 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60">{loading?<><Loader2 size={15} className="animate-spin"/>Aguarde...</>:mode==='login'?'Entrar com e-mail':'Criar minha conta'}</button>
      </form>}
    </div>
  </div>
}
