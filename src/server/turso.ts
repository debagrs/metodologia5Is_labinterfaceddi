...
type Statement = { sql: string; args?: TursoCell[] };
...
function arg(value: SqlValue): TursoCell {
...
async function pipeline(statements: Statement[]) {
...
function cellValue(cell: TursoCell | undefined): SqlValue {
...
export function validateSessionToken(rawHeader?: string | null): string | null {
...
export async function readWorkspace(ownerId: string): Promise<unknown | null> {
...
export async function saveWorkspace(ownerId: string, payload: unknown): Promise<void> {
  await ensureDatabase();
...
export async function consumeAiQuota(ownerId: string, dailyLimit: number): Promise<number> {
  await ensureDatabase();
