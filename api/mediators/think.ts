...
async function callGroq(system, user, deep) {
...
async function generateMediatorInsight(body) {
  ...
  const order = (process.env.AI_FALLBACK_ORDER || 'groq,gemini,offline')
    ...
    .filter((value) => ['groq', 'gemini', 'offline'].includes(value));
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    return res.status(500).json({ error: error?.message || 'Erro interno na mediação.', stage: 'mediator' });
  }
}
// @ts-nocheck
import crypto from 'node:crypto';
